...
try {
    ...
    // echo "Conexión correcta a la db";
...
function pointsFilters($scrap_title_less_5_words)
{
    ...
    return $scrap_title_less_5_words;
}
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if ($filtro === "points") {
        $results = pointsFilters($scrap_title_less_5_words);
        registrarLog($db, "points");
    } elseif ($filtro === "comments") {
        $results = commentsFilters($scrap_title_5_words);
        registrarLog($db, "comments");
    }
}
        ...
        <div class="result-box" style="text-align: center;">
            <?php foreach ($results as $r) { ?>
                <div class="result-item">
                    ...
                    <span class="comments">Comments: <?= $r["comment"] ?></span>
                </div>
            <?php } ?>
        </div>
    <?php } ?>
